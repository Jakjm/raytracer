...
//Function for parsing the input file provided to the program.
//@return -1 if the file could not be parsed without errors. 
//otherwise returns 0.
int parseFile(char *fileName){
	...
	//Reading in View planes
	result = fscanf(file, " NEAR %f LEFT %f RIGHT %f BOTTOM %f TOP %f ",&near,&left,&right,&bottom,&top);
	//Checking that the correct number of inputs were read.
	if(result != 5)return -1;


	//Screen resolution
	result = fscanf(file, " RES %d %d ",&cols,&rows);
	//Checking that the correct number of inputs were read.
	if(result != 2)return -1;

        //Reading spheres 
	result = fscanf(file," %s ",input);
	if(result != 1)return -1;//Ensuring at least one input was read. 

	while(strcmp("SPHERE",input) == 0){
		if(numSpheres >= 15){
			fprintf(stderr,"::Too many spheres. \n::There is a limit of 15 based on the given specifications.\n\n");
			return -1;
		}
		//Parse the sphere
		sphere *cS = readSphere(file);
		if(cS == NULL)return -1; //If the sphere failed to be read, break out of the method. 

		//Adding the current sphere to the array.
		sphereList[numSpheres] = cS;
		++numSpheres;
		result = fscanf(file," %s ",input);
    
		//In case the scan fails to read another item.             
		if(result != 1)return -1;
	}
		
	//By this point, input is currently "LIGHT" so we should skip reading it the first time. 
	//Reading lights...
	while(strcmp("LIGHT",input) == 0){
		if(numLights >= 10){
			fprintf(stderr,"::Too many lights. \n::There is a limit of 10 lights for this assignment.\n\n");
			return -1;
		}
		...
		//Adding the current light to the array.
		lightList[numLights] = l;
		++numLights;
		result = fscanf(file," %s ",input);
        
		//In case the scan returns EOF
		if(result != 1)return -1;
	}

	//By this point, input is currently "BACK" so we skip reading that.
	//Reading the back color of the scene. 
	result = fscanf(file, " %lf %lf %lf ",&r,&g,&b);
	if(result != 3)return -1;
         
	//Reading the ambient color of the scene.
	result = fscanf(file," AMBIENT %lf %lf %lf ",&aR,&aG,&aB);
	if(result != 3)return -1;

	//Reading the output file that this program will generate. 
	outputFile = malloc(sizeof(char) * 50);
	result = fscanf(file," OUTPUT %s",outputFile);
	if(result != 1)return -1;
